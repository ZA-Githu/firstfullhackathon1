...
_SYSTEM = (
    ...
    "Answer ONLY based on the provided context below. If the context does not contain "
    "enough information to answer the question, say so clearly and suggest what topics "
    "from the textbook might help.\n"
...
async def run_rag(
    ...
    history = await postgres.get_session_history(pool, session_id)
    ...
    # Filter out index/intro chunks with no meaningful section label
    meaningful_chunks = [
        c for c in chunks
